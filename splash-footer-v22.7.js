...
// - non-blocking (never // Archived reference snapshot — no functional change
// SPLASH FOOTER JS — V23.9 (Analytics Locked + Link Clicks Fixed)
// BASELINE: V23.8 (Analytics Locked)
// Adds: link_clicks logging via REST keepalive (constraint-safe: source + link_slot)
// - link_slot forced to 'A'/'B' only
...
async function insertLinkClickKeepalive(row){
  ...
  /* === BEGIN MINIMAL CHANGE: Non-music Title-Case for form entries only ===
     Constraint:
     - Apply title-casing ONLY for NON-music categories.
     - Do NOT affect music categories, album prediction, routing, analytics, or buttons.
     Why it cannot break existing behavior:
     - Runs only at submit-time inside the existing form submit handler.
     - Explicit no-op for parent === 'music'.
     - No new listeners; no changes to DOMContentLoaded structure or routing/analytics logic.
  === */
  function titleCaseNonMusicValue(category, value){
    const parent = getParentFromCategory(category);
    if (parent === 'music') return String(value || '').trim(); // explicit guard

    const v = String(value || '').trim();
    if (!v) return v;

    // Preserve separators like " - " while title-casing word tokens around them.
    return v
      .split(/(\s[-–—]\s)/)
      .map(part => (/^\s[-–—]\s$/.test(part) ? part : toTitleCase(part)))
      .join('')
      .trim();
  }

  function applyNonMusicTitleCase(category, values){
    const parent = getParentFromCategory(category);
    if (parent === 'music') return values; // hard no-op for music
    return (values || []).map(v => titleCaseNonMusicValue(category, v));
  }
  /* === END MINIMAL CHANGE: Non-music Title-Case for form entries only === */
  ...
  document.querySelectorAll('form').forEach((formEl) => {
    ...
    formEl.addEventListener('submit', async (event) => {
      ...
      // === BEGIN MINIMAL CHANGE (application point) ===
      // Title-case NON-music entries only; music categories remain untouched.
      newValues = applyNonMusicTitleCase(category, newValues);
      // === END MINIMAL CHANGE (application point) ===
