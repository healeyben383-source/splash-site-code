...
// - source forced to 'user_top5'// Archived reference snapshot — no functional change
// SPLASH FOOTER JS — V23.9 (Analytics Locked + Link Clicks Fixed)
// BASELINE: V23.8 (Analytics Locked)
// Adds: link_clicks logging via REST keepalive (constraint-safe: source + link_slot)
// - link_slot forced to 'A'/'B' only
...
// V24.0 — Title Case for NON-MUSIC form entries only
// - Applies on blur + before submit
// - Explicitly excluded for parent === 'music'
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* =========================
     V24.0 — TITLE CASE (NON-MUSIC ONLY)
     - Conservative title-casing that preserves internal punctuation
     - Skips empty strings
  ========================== */
  function titleCaseWords(str) {
    ...
    // Lowercase then capitalize first letter of each whitespace-separated token
    // Preserve hyphens/apostrophes inside tokens by capitalizing subparts
    return s
      .toLowerCase()
      .split(/\s+/)
      .filter(Boolean)
      .map(token => {
        // Split on hyphen/apostrophe boundaries but keep delimiters via capture groups
  ...
  function shouldApplyTitleCaseForCategory(category) {
    // Only apply to NON-music parent categories
    return getParentFromCategory(category) !== 'music';
  }

  function bindTitleCaseOnBlur(formEl, category) {
    if (!formEl || formEl.__SPLASH_TITLECASE_BOUND__) return;
    formEl.__SPLASH_TITLECASE_BOUND__ = true;

    if (!shouldApplyTitleCaseForCategory(category)) return;

    const inputs = [1,2,3,4,5]
      .map(i => formEl.querySelector(`input[name="rank${i}"]`))
      .filter(Boolean);

    inputs.forEach(input => {
      if (input.__SPLASH_TITLECASE_INPUT_BOUND__) return;
      input.__SPLASH_TITLECASE_INPUT_BOUND__ = true;

      input.addEventListener('blur', () => {
        // Do not interfere with prefilled flag mechanics
        const raw = String(input.value || '').trim();
        if (!raw) return;
        input.value = titleCaseWords(raw);
      });
    });
  }
  ...
  document.querySelectorAll('form').forEach((formEl) => {
    ...
    // V24.0 — bind titlecase on blur (NON-music only)
    bindTitleCaseOnBlur(formEl, category);
    ...
    formEl.addEventListener('submit', async (event) => {
      ...
      // Read values
      let newValues = [1,2,3,4,5].map(i =>
        (formEl.querySelector(`input[name="rank${i}"]`)?.value || '').trim()
      );

      // V24.0 — Apply Title Case right before validation/save (NON-music only)
      if (shouldApplyTitleCaseForCategory(category)) {
        newValues = newValues.map(v => {
          const t = String(v || '').trim();
          return t ? titleCaseWords(t) : '';
        });

        // Reflect back into inputs so UI matches saved values
        [1,2,3,4,5].forEach((i, idx) => {
          const input = formEl.querySelector(`input[name="rank${i}"]`);
          if (input && String(newValues[idx] || '').trim()) {
            input.value = newValues[idx];
          }
        });
      }
