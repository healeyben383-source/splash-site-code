...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function normalizeLinkClickPayload(payload){
    const p = payload && typeof payload === 'object' ? payload : {};
    const out = {};

    out.category = String(p.category || '').trim();
    out.display_name = String(p.display_name || p.display || '').trim();

    out.canonical_id = String(p.canonical_id || '').trim();
    if (!out.canonical_id && out.display_name) out.canonical_id = canonicalFromDisplay(out.display_name);

    out.link_slot = String(p.link_slot || '').trim().toUpperCase();
    if (out.link_slot) out.link_slot = out.link_slot[0];
    if (!out.link_slot) out.link_slot = 'A';

    out.link_label = String(p.link_label || '').trim() || 'Open';

    // IMPORTANT: must satisfy link_clicks_source_check in Supabase.
    // Keep this whitelist tight to avoid 400s from check constraint failures.
    const rawSource = String(p.source || '').trim();
    const ALLOWED_SOURCES = new Set(['user_top5', 'global_top100']);
    out.source = ALLOWED_SOURCES.has(rawSource) ? rawSource : 'user_top5';

    // Required fields (keep in sync with DB constraints)
    if (!out.category || !out.canonical_id || !out.display_name) return null;
    if (!out.link_slot || !out.link_label || !out.source) return null;
  ...
  function trackOutboundClick(payload){
    try {
      const clean = normalizeLinkClickPayload(payload);
      if (!clean) return;

      // De-dupe: pointerdown + click fallback can both fire in some browsers.
      const key = `${clean.category}|${clean.canonical_id}|${clean.link_slot}|${clean.link_label}|${clean.source}`;
      const now = Date.now();
      if (window.__SPLASH_LAST_LINK_CLICK_KEY__ === key && (now - (window.__SPLASH_LAST_LINK_CLICK_AT__ || 0)) < 900) return;
      window.__SPLASH_LAST_LINK_CLICK_KEY__ = key;
      window.__SPLASH_LAST_LINK_CLICK_AT__ = now;
      ...
      fetch(url, {
        ...
        if (DEBUG_LINK_CLICKS) {
          const ok = res && res.ok;
          console.log('[Splash] link_clicks POST', ok ? 'OK' : `FAIL ${res.status}`, clean);
          if (!ok) {
        ...
        if (DEBUG_LINK_CLICKS) console.warn('[Splash] link_clicks fetch failed', e, clean);
      });
  ...
  const MODAL_META_PARSED_KEY = '__diMetaParsed';
  ...
  function getModalMeta(sheet){
    ...
    if (sheet[MODAL_META_PARSED_KEY]) return sheet[MODAL_META_PARSED_KEY];
    const raw = sheet[MODAL_META_KEY] || sheet.dataset.diMeta || '';
    const parsed = safeJsonParse(raw) || parseLooseJsonObject(raw) || null;
    if (parsed) {
      sheet[MODAL_META_PARSED_KEY] = parsed;
      return parsed;
    }
    return null;
  }
...
a.__diMeta = meta || {};
    ...
    if (aUrl) body.appendChild(mkChoiceLink(aLabel || 'Link 1', aUrl, 'A'));
    if (bUrl) body.appendChild(mkChoiceLink(bLabel || 'Link 2', bUrl, 'B'));
